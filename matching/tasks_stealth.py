...
def get_next_user_for_scraping() -> Optional[int]:
    ...
    logger.info(
        f"🔄 Rotación de usuarios: Usando usuario {next_user_id} de {len(users_list)} verificados disponibles"
    )

    return next_user_id


async def _run_scraping_stealth(
    user_id: int, task_id: str = None, requesting_user_id: int = None
) -> Dict[str, Any]:
    """
    ...
    async def save_log(message: str, log_type: str = "info"):
        ...
        await sync_to_async(ScrapingLog.objects.create)(
            user_id=user_id, message=message, log_type=log_type, task_id=task_id
        )
    ...
    try:
        ...
        logger.info(
            f"👤 Usuario del Sistema: {user.get_full_name() or user.username} (ID: {user_id})"
        )
        ...
        if requesting_user_id and requesting_user_id != user_id:
            requesting_user = await sync_to_async(User.objects.get)(
            ...
            logger.info(
                f"🖥️  Solicitado por: {requesting_name} (ID: {requesting_user_id})"
            )
    ...
    try:
        if task_id:
            ...
            for worker, tasks in revoked_tasks.items():
                if task_id in tasks:
                    logger.info(
                        f"🛑 Tarea {task_id} fue cancelada, deteniendo scraping"
                    )
                    return {
                        "success": False,
                        "cancelled": True,
    ...
    try:
        ...
        if not login_result:
            ...
            try:
                ...
                logger.error(
                    f"❌ Login fallido para usuario {user_id} - Credenciales marcadas como NO VERIFICADAS"
                )
        ...
        for job_data in jobs:
            try:
                ...
                existing_job = await sync_to_async(
                    JobPosting.objects.filter(external_id=f"stealth_{unique_id}").first
                )()
        ...
        logger.info(
            f"Scraping stealth completado para usuario {user_id}: {new_jobs} nuevas, {saved_jobs} guardadas, {duplicated_jobs} duplicadas"
        )
        ...
        if saved_jobs > 0:  # Solo si se guardaron ofertas nuevas
            try:
                logger.info(
                    "🧮 Iniciando cálculo automático de matches para todos los usuarios..."
                )
                await save_log(
                    "🧮 Iniciando cálculo de matches para todos los usuarios...", "info"
                )
                ...
                match_task = await sync_to_async(
                    recalculate_matches_for_all_users.delay
                )()

                logger.info(f"✅ Tarea de cálculo de matches iniciada: {match_task.id}")
                await save_log(
                    f"✅ Cálculo de matches iniciado en background (task: {match_task.id})",
                    "success",
                )
                ...
                await save_log(
                    f"⚠️ No se pudo iniciar cálculo automático de matches: {str(e)}",
                    "warning",
                )
        else:
            logger.info(
                "ℹ️ No se guardaron ofertas nuevas, omitiendo cálculo de matches"
            )
...
def scrape_dvcarreras_jobs_stealth(
    self, user_id: int = None, requesting_user_id: int = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if user_id is None:
            ...
            if not user_id:
                logger.error(
                    "No hay usuarios con credenciales disponibles para scraping"
                )
                return {
                    "success": False,
        ...
        result = asyncio.run(
            _run_scraping_stealth(
                user_id, task_id=task_id, requesting_user_id=requesting_user_id
            )
        )
        ...
        try:
            ...
            if released:
                ...
                logger.warning(f"⚠️ release_lock() retornó False para task={task_id}")
        except Exception as e:
        ...
        logger.info(
            f"🔓 [FINALLY] Finalizado intento de liberación de lock (task={task_id})"
        )
...
def test_stealth_login(self, user_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "message": f"Error: {str(e)}", "user_id": user_id}
...
def check_and_run_scheduled_scraping(self):
    ...
    try:
        ...
        if scraping_lock.is_locked():
            ...
            logger.info(
                f"⏸️ Scraping programado omitido: ya hay un scraping activo (task={task_id})"
            )
            return {
                "success": False,
        ...
        if config.last_run and not schedule_changed:
            ...
            if last_run_date == today:
                logger.info(
                    f"✋ Scraping ya ejecutado hoy a las {last_run_local.strftime('%H:%M:%S')}"
                )
                return {
                    "success": False,
        ...
        # Ejecutar scraping
        logger.info(
            f"🕒 Iniciando scraping programado a las {local_now.strftime('%H:%M:%S')}"
        )
