...
def generate_personalized_cv(request):
    ...
    try:
        logger.info(f"🔍 Iniciando personalización de CV - Usuario: {request.user.username}")
        ...
        if not cv_id or not job_id:
            ...
            return JsonResponse({
                'success': False,
                'error': 'CV ID y Job ID son requeridos'
            })
        ...
        logger.info(f"✅ CV encontrado: {user_cv.original_file.name if user_cv.original_file else 'Sin archivo'}")
        ...
        if result['success']:
            logger.info(f"✅ CV personalizado generado exitosamente - Score: {result['match_score']}%")
            response_data = {
                'success': True,
...
def cv_personalization_analytics(request):
    ...
    for cv in user_cvs:
        ...
        cv_stat = {
            'cv_id': cv.id,
            'filename': cv.original_file.name.split('/')[-1] if cv.original_file else 'CV',
            'skills_count': len(cv.skills_list),
            'matches_count': matches.count(),
            'avg_match_score': matches.aggregate(avg_score=models.Avg('score'))['avg_score'] or 0,
    ...
    avg_skills_per_cv = sum(stat['skills_count'] for stat in cv_stats) / total_cvs if total_cvs > 0 else 0
...
def cv_personalization_history(request):
    ...
    context = {
        'page_obj': page_obj,
        'page_title': 'Historial de Personalización de CV'
    }
        ...
        if result['success'] and result['personalized_file']:
            ...
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    ...
                    filename = f"CV_personalizado_{job_posting.title.replace(' ', '_')}.pdf"
