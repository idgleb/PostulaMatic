...
def email_log_detail(request, log_id):
    ...
    email_log = get_object_or_404(EmailSentLog, id=log_id, user=request.user)
...
def send_test_email(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Error enviando email: {str(e)}"}
        )
...
def send_bulk_emails(request):
    ...
    try:
        ...
        if not job_ids:
            return JsonResponse(
                {"success": False, "error": "No se especificaron puestos"}
            )
        ...
        return JsonResponse(
            {"success": False, "error": f"Error iniciando envío masivo: {str(e)}"}
        )
...
def process_auto_matching(request):
    ...
    try:
        ...
        user_cv = (
            UserCV.objects.filter(user=request.user, parsed_text__isnull=False)
            .exclude(parsed_text="")
            .first()
        )
        if not user_cv:
...
def email_statistics(request):
    ...
    top_jobs = (
        EmailSentLog.objects.filter(user=request.user)
        .values("job_posting__title", "job_posting__company")
        .annotate(count=Count("id"))
        .order_by("-count")[:10]
    )
...
def task_status(request, task_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Error obteniendo estado de tarea: {str(e)}"}
        )
...
def cleanup_email_logs(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Error iniciando limpieza: {str(e)}"}
        )
    ...
    recent_emails_data = [
        {
            "id": email.id,
            "job_title": (
                email.job_posting.title if email.job_posting else "[Oferta eliminada]"
            ),
            ...
            "cv_filename": (
                email.cv.original_file.name.split("/")[-1]
                if email.cv and email.cv.original_file
                else "[CV eliminado]"
            ),
        }
        ...
        emails_data.append(
            {
                "id": email.id,
                "job_title": (
                    email.job_posting.title
                    if email.job_posting
                    else "[Oferta eliminada]"
                ),
                "sent_to": email.sent_to,
                "status": email.status,
    ...
    return JsonResponse(
        {
            ...
            "pagination": {
                ...
                "previous_page": (
                    emails_page.previous_page_number()
                    if emails_page.has_previous()
                    else None
                ),
                "next_page": (
                    emails_page.next_page_number() if emails_page.has_next() else None
                ),
    ...
    cvs_data = [
        {
            ...
            "skills_count": (
                cv.skills_count if hasattr(cv, "skills_count") else len(cv.skills_list)
            ),
...
def get_email_detail(request, email_id):
    ...
    try:
        ...
        email_data = {
            ...
            "personalized_cv_filename": (
                email.personalized_cv_file.name.split("/")[-1]
                if email.personalized_cv_file
                else None
            ),
        ...
        return JsonResponse(
            {"success": False, "message": "Email no encontrado"}, status=404
        )
    except Exception as e:
        return JsonResponse(
            {
...
def download_personalized_cv(request, email_id):
    ...
    try:
        email = get_object_or_404(EmailSentLog, id=email_id, user=request.user)
...
def delete_email(request, email_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Email eliminado correctamente"}
        )

    except EmailSentLog.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": "Email no encontrado"}, status=404
        )
    except Exception as e:
