...
class FlareSolverrClient:
    ...
    def __init__(self, base_url: str = None):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "PostulaMatic/1.0"}
        )
    ...
    def solve_post(
        self, url: str, post_data: str, user_agent: str = None, max_timeout: int = 60000
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_cookies(
        self, url: str, user_agent: str = None, max_timeout: int = 60000
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    try:
        ...
        if result and result.get("status") == "ok":
            return {
                ...
                "response": (
                    result.get("solution", {}).get("response", "")[:500] + "..."
                    if len(result.get("solution", {}).get("response", "")) > 500
                    else result.get("solution", {}).get("response", "")
                ),
            }
        else:
            return {
        ...
        return {"success": False, "message": f"Error probando FlareSolverr: {str(e)}"}
