...
class CVDataExtractor:
    ...
    def extract_cv_data(user_cv: UserCV) -> Dict:
        ...
        try:
            ...
            experience_info = CVDataExtractor._extract_experience_info(
                user_cv.parsed_text
            )

            # Extraer educación
            education_info = CVDataExtractor._extract_education_info(
                user_cv.parsed_text
            )
            ...
            if any(keyword in line_lower for keyword in experience_keywords):
                ...
                if role_parts and found_techs:
                    summary_parts.append(
                        f"{role_parts[0]} {', '.join(found_techs[:2])}"
                    )
                elif role_parts:
    ...
    def _extract_projects_info(parsed_text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(keyword in line_lower for keyword in education_keywords):
                continue
            if any(
                keyword in line_lower for keyword in experience_keywords
            ) and not any(
                proj_keyword in line_lower for proj_keyword in ["proyecto", "project"]
            ):
                continue
        ...
        if not projects:
            for line in lines:
                ...
                if (
                    not any(keyword in line_lower for keyword in education_keywords)
                    and not any(
                        keyword in line_lower for keyword in experience_keywords
                    )
                    and any(keyword in line_lower for keyword in project_keywords)
...
class JobDataExtractor:
    ...
    def _extract_experience_level(description: str) -> str:
        ...
        years_match = re.search(
            r"(\d+)\+?\s*(?:años?|years?)\s*(?:de\s*)?experiencia", description_lower
        )
        if years_match:
...
class EmailPersonalizationService:
    ...
    def _select_optimal_template(
        self, cv_data: Dict, job_data: Dict, requested_template: str
    ) -> str:
        ...
        if (
            len(job_data["description"]) < 500
        ):  # Descripciones cortas suelen ser startups
            return "startup"
