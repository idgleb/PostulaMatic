...
class PDFParser:
    ...
    def parse_cv(self, file_path: str) -> Dict:
        ...
        if not self.is_supported(file_path):
            return {
                'text': '',
                'warning_message': 'Formato PDF no soportado'
            }
        ...
        try:
            with open(file_path, 'rb') as file:
                ...
                if not pdf_reader.pages:
                    return {
                        'text': '',
                        'warning_message': 'PDF sin páginas'
                    }
            ...
            return {
                'text': '',
                'warning_message': f'Error parseando PDF: {str(e)}'
            }
    ...
    def _clean_pdf_text(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if (len(line) > 3 and 
                not re.match(r'^[\d\s.,\-+]+$', line) and  # No solo números
                not re.match(r'^[0-9.\s]+$', line) and     # No solo decimales
                'cm' not in line and                       # No coordenadas
                'RG' not in line and                       # No colores
                'J' not in line and                        # No comandos PDF
                'm' not in line or 'l' not in line):       # No comandos de línea
