...
class AIPDFParser:
    ...
    def parse_cv(self, file_path: str, progress_tracker=None) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"🤖 AIPDFParser: Iniciando extracción con IA para: {file_path}")
            logger.info("🤖 Flujo: OpenAI (visión) → Anthropic (visión) → Error si ambos fallan")
            ...
            if progress_tracker:
                progress_tracker.update_step("pdf_to_images", "completed", f"{len(pdf_images)} imágenes extraídas")
            logger.info(f"📋 PASO 1 COMPLETADO: Se extrajeron {len(pdf_images)} imágenes del PDF")
            ...
            for i, image_base64 in enumerate(pdf_images):
                ...
                try:
                    ...
                    logger.info(f"🔍 PASO 2.{i+1}: Texto extraído de página {i+1}: {len(page_text)} caracteres")
                    ...
                    if page_text:
                        ...
                        logger.info(f"🔍 PASO 2.{i+1}: Página {i+1} agregada a resultados")
                    else:
                        logger.warning(f"🔍 PASO 2.{i+1}: Página {i+1} no produjo texto")
                    if used_fallback:
                        fallback_used = True
                        logger.info(f"🔄 PASO 2.{i+1}: Fallback detectado en página {i+1}")
                except Exception as e:
                    # Si hay error en esta página, NO intentar con la siguiente
                    # Lanzar el error completo inmediatamente para que se propague
                    logger.error(f"❌ PASO 2.{i+1}: Error procesando página {i+1}, abortando: {e}")
                    if progress_tracker:
                        step_id = "anthropic_vision" if fallback_used else "openai_vision"
                        progress_tracker.set_error(str(e), step_id)
                    raise e  # Re-lanzar el error completo sin modificarlo
            
            logger.info(f"📋 PASO 2 COMPLETADO: Se procesaron {len(extracted_texts)} páginas exitosamente")
            ...
            logger.info(f"📋 PASO 3 COMPLETADO: Texto combinado: {len(full_text)} caracteres")
            
            # 4. Post-procesar con IA para mejorar estructura (DESACTIVADO para evitar timeouts)
            logger.info("📋 PASO 4: Saltado temporalmente (se devuelve texto crudo extraído)")
            improved_text = full_text
            
            logger.info(f"✅ Extracción con IA completada: {len(improved_text)} caracteres")
            ...
            if "OpenAI" in str(e) or "Anthropic" in str(e) or "visión" in str(e).lower():
    ...
    def _pdf_to_images(self, file_path: str) -> list:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                logger.info(f"🔍 PIXMAP CREADO PARA PÁGINA {page_num + 1}: {pix.width}x{pix.height}")
                
                img_data = pix.tobytes("png")
                logger.info(f"🔍 IMAGEN PNG GENERADA PARA PÁGINA {page_num + 1}: {len(img_data)} bytes")
                
                # Convertir a base64
                img_base64 = base64.b64encode(img_data).decode('utf-8')
                logger.info(f"🔍 BASE64 GENERADO PARA PÁGINA {page_num + 1}: {len(img_base64)} caracteres")
    ...
    def _extract_text_from_image_with_ai(self, image_base64: str, progress_tracker=None) -> tuple:
        """
        ...
        try:
            ...
            try:
                response, used_fallback = self.ai_service.generate_with_vision_and_track_fallback(prompt, image_base64, progress_tracker)
                
                if used_fallback:
                    logger.info(f"✅ ANTHROPIC EXITOSO (FALLBACK): Respuesta recibida de {len(response)} caracteres")
                    logger.info(f"✅ ANTHROPIC PREVIEW: {response[:200]}...")
                else:
                    logger.info(f"✅ OPENAI EXITOSO: Respuesta recibida de {len(response)} caracteres")
...
"""
            ...
            try:
                response, fallback_used = self.ai_service.generate_text_and_track_fallback(prompt)
            except Exception as e:
                logger.error(f"❌ Post-procesamiento con IA falló, devolviendo texto crudo: {e}")
                return text, False
            ...
            if "IA no disponible" in str(e) or "Cuota agotada" in str(e) or "API keys inválidas" in str(e):
                raise Exception(f"❌ IA no disponible - Cuota agotada o API keys inválidas")
            else:
                return text, False
