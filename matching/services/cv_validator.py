...
class CVValidator:
    ...
    def validate_cv_content(self, text: str, filename: str = "") -> Dict[str, Any]:
        ...
        if not text or len(text.strip()) < 50:
            return {
                ...
                "suggestions": [
                    "Verifica que el archivo sea un CV válido",
        ...
        if (
            financial_score > 0.5
        ):  # Reducido el umbral para detectar mejor documentos financieros
            return {
                "is_valid": False,
                "confidence": 1.0 - financial_score,
                "reason": "El archivo parece ser un documento financiero/nómina, no un CV",
                "suggestions": [
                    "Sube tu CV profesional en lugar de documentos de nómina",
                    "El archivo debe contener información sobre tu experiencia y habilidades",
                ],
            }
        ...
        if technical_score > 0.6:
            return {
                ...
                "suggestions": [
                    "Sube tu CV profesional",
                    "El archivo debe contener información sobre tu perfil profesional",
                ],
            }
        ...
        if cv_score < 0.3:
            return {
                ...
                "suggestions": [
                    "Asegúrate de que el archivo sea tu CV profesional",
                    "El CV debe incluir secciones como experiencia, educación y habilidades",
                ],
            }

        # Calcular confianza general
        confidence = min(
            cv_score + (1 - financial_score) * 0.3 + (1 - technical_score) * 0.2, 1.0
        )
    ...
    def _calculate_financial_score(self, text: str, filename: str) -> float:
        ...
        if any(
            word in filename
            for word in ["payroll", "nómina", "banco", "cuenta", "salario"]
        ):
            score += 0.3
