...
class ScrapingLockService:
    ...
    @staticmethod
    def acquire_lock(task_id: str, user_id: Optional[int] = None, source: str = 'manual') -> bool:
        """
        ...
        try:
            ...
            lock_acquired = cache.add(SCRAPING_LOCK_KEY, task_id, timeout=SCRAPING_LOCK_TIMEOUT)
            ...
            if lock_acquired:
                ...
                logger.info(f"🔒 Lock de scraping adquirido: task={task_id}, user={user_id}, source={source}")
                return True
            else:
    ...
    def release_lock(task_id: str) -> bool:
        ...
        try:
            ...
            if current_lock == task_id:
                ...
                logger.warning(f"⚠️ Intento de liberar lock sin ser dueño: task={task_id}, lock_actual={current_lock}")
                return False
