...
class BasicSkillsMatchingStrategy(MatchingStrategy):
    ...
    def calculate_match(
        ...
        job_skills_data = skills_extractor.extract_skills(
            job_description, min_confidence=0.5
        )
...
class MatchingService:
    ...
    def calculate_user_job_matches(
        ...
        for cv in user_cvs:
            try:
                ...
                logger.error(
                    f"Error calculando match para CV {cv.id} y job {job.id}: {e}"
                )
                continue
    ...
    def save_match_score(
        self, user: UserProfile, cv: UserCV, job: JobPosting, match_result: MatchResult
    ) -> MatchScore:
        """
        ...
        if created:
            logger.info(
                f"Nuevo match score creado: {match_result.score}% para job {job.id}"
            )
        else:
            logger.info(
                f"Match score actualizado: {match_result.score}% para job {job.id}"
            )

        return match_score

    def get_high_matches(
        self, user_profile: UserProfile, threshold: float = 70.0
    ) -> List[MatchScore]:
        """
        ...
        return MatchScore.objects.filter(
            user=user_profile.user, score__gte=threshold
