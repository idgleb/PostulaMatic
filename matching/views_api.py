...
def user_cvs_api(request):
    ...
    for cv in cvs:
        cvs_data.append({
            ...
            'file_name': cv.original_file.name.split('/')[-1] if cv.original_file else 'Sin archivo'
        })
...
def job_postings_api(request):
    ...
    jobs = JobPosting.objects.all().order_by('-created_at')[:100]  # Limitar a 100 más recientes
...
def email_statistics_api(request):
    ...
    successful_emails = EmailSentLog.objects.filter(user=request.user, status='sent').count()
    failed_emails = EmailSentLog.objects.filter(user=request.user, status='failed').count()
