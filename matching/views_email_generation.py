...
def email_generation_test_view(request):
    ...
    user_cvs = UserCV.objects.filter(user=request.user, parsed_text__isnull=False).exclude(parsed_text='')
    ...
    ).select_related('job_posting', 'user_cv').order_by('-score')[:10]
    ...
    context = {
        ...
        'available_templates': [
            'base', 'formal', 'creative', 'technical', 'startup', 'corporate'
        ],
...
def generate_test_email_view(request):
    ...
    try:
        ...
        if not cv_id or not job_id:
            return JsonResponse({
                'success': False,
                'message': 'CV y puesto de trabajo son requeridos'
            })
