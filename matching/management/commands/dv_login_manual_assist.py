...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"🚀 Iniciando login manual asistido para usuario: {username}")
        self.stdout.write("📋 Este comando abrirá un navegador donde podrás resolver el CAPTCHA manualmente")
    ...
    async def _manual_login(self, username, sessions_dir):
        ...
        try:
            ...
            if not password:
                raise CommandError(f"Usuario {username} no tiene contraseña DV configurada")
            
            # Crear cliente Playwright en modo headful (visible)
            client = DVCarrerasPlaywrightFlareSolverr(username, password, headless=False)
            ...
            try:
                ...
                self.stdout.write("⏳ Esperando a que resuelvas el CAPTCHA manualmente...")
                self.stdout.write("💡 Una vez que veas la página de dashboard, presiona Enter aquí...")
                
                # Esperar input del usuario
                input("Presiona Enter cuando hayas resuelto el CAPTCHA y estés en el dashboard...")
                ...
                self.stdout.write("🔄 Ahora puedes usar la verificación automática normal")
