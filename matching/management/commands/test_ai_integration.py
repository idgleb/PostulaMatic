...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--user-id", type=int, help="ID del usuario para usar sus datos"
        )
        parser.add_argument("--cv-id", type=int, help="ID del CV específico a usar")
        parser.add_argument(
            "--job-id", type=int, help="ID del puesto específico a usar"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("✅ Pruebas de integración completadas"))
    ...
    def check_configuration(self):
        ...
        if openai_key:
            ...
            self.stdout.write(self.style.WARNING("⚠️ OpenAI API Key no configurada"))

        # Verificar Anthropic
        anthropic_key = os.getenv("ANTHROPIC_API_KEY") or getattr(
            settings, "ANTHROPIC_API_KEY", ""
        )
        if anthropic_key:
            self.stdout.write(self.style.SUCCESS("✅ Anthropic API Key configurada"))
        else:
            self.stdout.write(self.style.WARNING("⚠️ Anthropic API Key no configurada"))
    ...
    def get_test_data(self, options):
        ...
        if options["cv_id"]:
            try:
                ...
                raise CommandError(
                    f'CV con ID {options["cv_id"]} no encontrado para el usuario'
                )
        else:
            user_cv = (
                UserCV.objects.filter(user=user, parsed_text__isnull=False)
                .exclude(parsed_text="")
                .first()
            )
        ...
        self.stdout.write(
            f"📄 CV: {user_cv.original_file.name} ({user_cv.skills_count} habilidades)"
        )
        ...
        if options["job_id"]:
            ...
            job_posting = (
                JobPosting.objects.filter(description__isnull=False)
                .exclude(description="")
                .first()
            )
    ...
    def test_openai(self, user, user_cv, job_posting):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Error probando OpenAI: {e}"))
    ...
    def test_anthropic(self, user, user_cv, job_posting):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Error probando Anthropic: {e}"))
