...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Iniciando prueba del scraper stealth para usuario {user_id}...')
        )
    ...
    async def test_stealth_scraper(self, user_id: int, headless: bool):
        ...
        try:
            ...
            if not await client.start():
                self.stdout.write(
                    self.style.ERROR('Error iniciando navegador')
                )
                return
            
            self.stdout.write(
                self.style.SUCCESS('Navegador iniciado correctamente')
            )
            ...
            if await client.login():
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Scraping completado: {len(jobs)} ofertas encontradas')
                )
                ...
                for i, job in enumerate(jobs, 1):
                    ...
                    self.stdout.write(f'Descripción: {job.get("description", "N/A")[:100]}...')
                    self.stdout.write(f'Email: {job.get("email_text", "N/A")}')
                    
            else:
                self.stdout.write(
                    self.style.ERROR('Login fallido')
                )
