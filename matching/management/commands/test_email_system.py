...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user_cv:
            self.stdout.write(
                self.style.ERROR('❌ El usuario no tiene CV')
            )
            return
        ...
        self.stdout.write(f'🔧 Configuración: {test_type}, template: {email_template}, IA: {ai_provider}')
    ...
    def test_single_email(self, user, user_cv, email_template, ai_provider):
        ...
        try:
            ...
            if result['success']:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Email enviado exitosamente: {result["message"]}')
                )
            else:
                self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Error esperando resultado: {e}')
            )
    ...
    def test_bulk_emails(self, user, user_cv, email_template, ai_provider):
        ...
        try:
            ...
            if result['success']:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Emails masivos enviados: {result["message"]}')
                )
                self.stdout.write(f'📊 Total: {result["total_jobs"]}, Exitosos: {result["successful_queued"]}, Fallidos: {result["failed"]}')
            else:
                self.stdout.write(
                    self.style.ERROR(f'❌ Error: {result["error"]}')
                )
    ...
    def test_auto_matching(self, user, user_cv, email_template, ai_provider):
        ...
        try:
            ...
            if result['success']:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Matching automático completado: {result["message"]}')
                )
    ...
    def show_statistics(self, user):
        ...
        successful_emails = EmailSentLog.objects.filter(user=user, status='sent').count()
