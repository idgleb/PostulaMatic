...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Iniciando prueba de gestión de sesiones..."))
            ...
            try:
                ...
                if login_result:
                    ...
                    if client.session_file.exists():
                        self.stdout.write(self.style.SUCCESS("✅ Sesión guardada correctamente"))
                    else:
                ...
                if session_loaded:
                    self.stdout.write(self.style.SUCCESS("✅ Sesión cargada correctamente"))
                    
                    # Verificar validez
                    if await client2.test_session_validity():
                        self.stdout.write(self.style.SUCCESS("✅ Sesión válida confirmada"))
                    else:
                        self.stdout.write(self.style.WARNING("⚠️ Sesión no es válida"))
                else:
                    self.stdout.write(self.style.WARNING("⚠️ No se pudo cargar la sesión"))
                ...
                if client.session_file.exists():
                    ...
                    if client.cookies_file.exists():
                        self.stdout.write(self.style.SUCCESS("✅ Archivo de cookies también existe"))
                    else:
                        self.stdout.write(self.style.WARNING("⚠️ Archivo de cookies no existe"))
