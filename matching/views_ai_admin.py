    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.info(
                    f"🔍 Datos del formulario - OpenAI model: {form.cleaned_data.get('openai_model')}, Enabled: {form.cleaned_data.get('openai_enabled')}"
                )

                # Log antes de guardar
                logger.info(
                    f"🔍 Antes de guardar - OpenAI model: {config.openai_model}, Enabled: {config.openai_enabled}"
                )
                ...
                logger.info(
                    f"🔍 Después de guardar - OpenAI model: {saved_config.openai_model}, Enabled: {saved_config.openai_enabled}"
                )

                messages.success(
                    request, "✅ Configuración de IA actualizada correctamente"
                )
...
def test_ai_provider_admin(request):
    ...
    try:
        ...
        if not provider:
            return JsonResponse(
                {"success": False, "error": "Proveedor no especificado"}
            )
        ...
        if provider == "openai" and not (
            config.openai_enabled and config.openai_api_key
        ):
            return JsonResponse(
        ...
        if provider == "anthropic" and not (
            config.anthropic_enabled and config.anthropic_api_key
        ):
            return JsonResponse(
                {"success": False, "error": "Anthropic no está configurado"}
            )
        ...
        if result.error:
            ...
            if any(
                ...
                keyword in error_lower
                for keyword in ["401", "invalid", "api_key", "authentication"]
            ):
                error_msg = f"❌ {provider.upper()}: API key inválida o no autorizada."
            elif any(
                keyword in error_lower for keyword in ["403", "forbidden", "permission"]
            ):
            ...
            return JsonResponse(
                {"success": False, "error": error_msg, "provider": provider}
            )
...
def ai_admin_status_view(request):
    ...
    providers_status["anthropic"] = {
        ...
        "enabled": config.anthropic_enabled,
        "key_preview": (
            f"{anthropic_key[:12]}..." if anthropic_key else "No configurado"
        ),
        "status": (
            "active" if (config.anthropic_enabled and anthropic_key) else "inactive"
        ),
    }
        ...
        if config.openai_api_key:
            try:
                ...
                for model in models_response.data:
                    ...
                    if any(
                        prefix in model_id for prefix in ["gpt-3.5", "gpt-4", "gpt-4o"]
                    ):
                        ...
                        chat_models.append(
                            {
                                ...
                                "name": display_name,
                                "created": (
                                    model.created if hasattr(model, "created") else None
                                ),
                            }
                        )
        ...
        if config.anthropic_api_key:
            try:
                ...
                for model_id in available_models:
                    ...
                    anthropic_models.append({"id": model_id, "name": display_name})

                result["anthropic"]["models"] = anthropic_models
                logger.info(
                    f"✅ Modelos Anthropic disponibles: {len(anthropic_models)}"
                )
