...
class UserProfileForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'match_threshold', 'daily_limit', 
            'min_pause_seconds', 'max_pause_seconds'
        ]
        widgets = {
            'smtp_password': forms.PasswordInput(attrs={'placeholder': 'Contraseña SMTP'}),
            'dv_password': forms.PasswordInput(attrs={'placeholder': 'Contraseña dvcarreras'}),
    ...
    def clean(self):
        ...
        if smtp_use_tls and smtp_use_ssl:
            raise ValidationError("No se puede usar TLS y SSL simultáneamente. Elige solo uno.")
...
class SMTPConfigForm(forms.ModelForm):
    ...
    class Meta:
        model = UserProfile
        fields = ['display_name', 'smtp_host', 'smtp_port', 'smtp_use_tls', 'smtp_use_ssl', 'smtp_username', 'smtp_password']
        widgets = {
            'smtp_host': forms.TextInput(attrs={'class': 'form-control', 'required': True}),
            'smtp_port': forms.NumberInput(attrs={'min': 1, 'max': 65535, 'class': 'form-control', 'required': True}),
            'smtp_username': forms.EmailInput(attrs={'class': 'form-control', 'required': True}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.smtp_password:
            self.fields['smtp_password'].widget.attrs['placeholder'] = '•••••••• (contraseña guardada)'
...
class DVCredentialsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'dv_username': forms.TextInput(attrs={'class': 'form-control', 'required': True}),
            'dv_password': forms.PasswordInput(attrs={
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.dv_password:
            self.fields['dv_password'].widget.attrs['placeholder'] = '•••••••• (contraseña guardada)'
