...
def _recalculate_matches_for_user_core(user_id: int):
    ...
    try:
        ...
        if not jobs.exists():
            logger.warning(f"No hay ofertas de trabajo para calcular matches para usuario {user_id}")
            return {
                "success": True,
                "user_id": user_id,
                "old_matches_count": 0,
        ...
        if not user_cvs.exists():
            logger.warning(f"Usuario {user_id} no tiene CVs procesados para calcular matches")
            return {
                "success": True,
                "user_id": user_id,
                "old_matches_count": 0,
        ...
        logger.info(f"Eliminando {total_matches_before} matches existentes para recalcular con nuevo umbral")
            ...
            try:
                ...
                for cv, match_result in matches:
                    ...
                    if match_result.score >= user_profile.match_threshold:
                        ...
                        logger.debug(f"Match creado: {match_result.score}% para job {job.id} con CV {cv.id}")
                    else:
                        # Este match no supera el umbral - no se guarda
                        logger.debug(f"Match descartado: {match_result.score}% < {user_profile.match_threshold}% para job {job.id}")
