...
from .models import UserProfile, JobPosting, MatchScore, UserCV, ScrapingLog
from .clients.dvcarreras import DVCarrerasClient, JobPostingData
...
def create_or_update_job(external_id, defaults):
    return JobPosting.objects.update_or_create(external_id=external_id, defaults=defaults)

async_create_or_update_job = sync_to_async(create_or_update_job)
async_get_user_profile = sync_to_async(lambda user_id: UserProfile.objects.get(user_id=user_id))
...
def scrape_dvcarreras_jobs(self, user_id: int):
    ...
    try:
        ...
        if not user_profile.dv_username or not user_profile.dv_password:
            logger.error(f"Usuario {user_id} no tiene credenciales de INTRANET DAVINCI configuradas")
        ...
        with DVCarrerasClient(
            ...
            logger.info(f"Encontradas {len(job_postings_data)} ofertas para usuario {user_id}")
            ...
            for job_data in job_postings_data:
                try:
                    ...
                    if created:
                        new_jobs += 1
                        logger.info(f"Nueva oferta guardada: {job_posting.title} - Email: {job_posting.email}")
                    
                    # Calcular matches con CVs del usuario
                    matches = matching_service.calculate_user_job_matches(user_profile, job_posting)
                    
                    for cv, match_result in matches:
                        # Solo guardar si supera el umbral
                        if match_result.score >= user_profile.match_threshold:
                            matching_service.save_match_score(user_profile.user, cv, job_posting, match_result)
                            logger.info(f"Match encontrado: {match_result.score}% para {job_posting.title}")
        ...
        if self.request.retries < self.max_retries:
            logger.info(f"Reintentando scraping para usuario {user_id} (intento {self.request.retries + 1})")
...
def process_new_job_postings():
    ...
    try:
        ...
        ).exclude(dv_username='').exclude(dv_password='')
        ...
        for user_profile in active_users:
            try:
                ...
                logger.error(f"Error iniciando scraping para usuario {user_profile.user.id}: {e}")
                results.append({
                    'user_id': user_profile.user.id,
                    'error': str(e)
                })
...
def calculate_matches_for_job(job_id: int):
    ...
    try:
        ...
        active_users = UserProfile.objects.filter(
            is_active=True,
            user__cvs__parsed_text__isnull=False
        ).exclude(user__cvs__parsed_text='').distinct()
        ...
        for user_profile in active_users:
            try:
                ...
                logger.error(f"Error calculando matches para usuario {user_profile.user.id}: {e}")
                continue
...
def process_cv_file(self, cv_id: int):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"CV {cv_id} procesado exitosamente: {cv.skills_count} skills detectadas")
            ...
            if self.request.retries < self.max_retries:
                logger.info(f"Reintentando procesamiento de CV {cv_id} (intento {self.request.retries + 1})")
                raise self.retry(countdown=60 * (self.request.retries + 1))
    ...
    try:
        ...
        for cv in pending_cvs:
            try:
                ...
                processed_count += 1
                logger.info(f"Tarea de procesamiento enviada para CV {cv.id}: {task_result.id}")
...
def scrape_dvcarreras_jobs_advanced(self, user_id: int):
    ...
    """
    try:
        logger.info(f"Iniciando scraping AVANZADO de INTRANET DAVINCI para usuario {user_id}")
        ...
        # Reintentar si no es el último intento
        if self.request.retries < self.max_retries:
            logger.info(f"Reintentando scraping AVANZADO para usuario {user_id} (intento {self.request.retries + 1})")
            raise self.retry(countdown=60 * (self.request.retries + 1))
...
def scrape_dvcarreras_jobs_playwright(self, user_id: int):
    ...
    try:
        ...
        import asyncio
        asyncio.run(async_save_scraping_log(user_id, self.request.id, 'Worker iniciando proceso de scraping', 'info'))
        ...
        async def run_playwright_scraping():
            ...
            # Guardar log en la base de datos
            await async_save_scraping_log(user_id, self.request.id, 'Iniciando navegador Playwright', 'info')
            ...
            async with DVCarrerasPlaywrightSimple(
                ...
                # Guardar log en la base de datos
                await async_save_scraping_log(user_id, self.request.id, 'Navegando al portal', 'info')
                await async_save_scraping_log(user_id, self.request.id, 'Accediendo a la bolsa de trabajo', 'info')
                ...
                # Guardar log en la base de datos
                await async_save_scraping_log(user_id, self.request.id, 'Extrayendo ofertas', 'info')
                await async_save_scraping_log(user_id, self.request.id, 'Scrapeando ofertas de trabajo', 'info')
                ...
                await async_save_scraping_log(user_id, self.request.id, f'Ofertas encontradas: {len(job_postings_data)}', 'info')
                ...
                try:
                    ...
                    logger.warning(f"No se pudo calcular únicas/duplicadas en lote: {_e}")
                ...
                # Guardar log en la base de datos
                await async_save_scraping_log(user_id, self.request.id, 'Procesando datos', 'info')
                await async_save_scraping_log(user_id, self.request.id, 'Analizando ofertas encontradas', 'info')
                ...
                for job_data in job_postings_data:
                    try:
                        logger.info(f"Verificando duplicado para: {job_data.title} - Email: {job_data.email}")
                        ...
                        if created:
                            ...
                            logger.info(f"Oferta duplicada omitida: {job_posting.title} - Email: {job_posting.email}")
                        
                        # Calcular matches con CVs del usuario usando función async
                        matches = await async_calculate_matches(user_profile, job_posting)
                        
                        for cv, match_result in matches:
                            # Solo guardar si supera el umbral
                            if match_result.score >= user_profile.match_threshold:
                                await async_save_match_score(user_profile.user, cv, job_posting, match_result)
                                matches_found += 1
                                logger.info(f"Match encontrado: {match_result.score}% para {job_posting.title}")
                ...
                await async_save_scraping_log(user_id, self.request.id, f'Ofertas nuevas: {new_jobs}', 'info')
                ...
                # Guardar log en la base de datos
                await async_save_scraping_log(user_id, self.request.id, 'Finalizando', 'info')
                await async_save_scraping_log(user_id, self.request.id, 'Completando proceso de scraping', 'info')
                
                # Guardar resumen final con datos correctos
                await async_save_scraping_log(user_id, self.request.id, '¡Scraping completado exitosamente!', 'success')
                await async_save_scraping_log(user_id, self.request.id, f'📊 Resumen: {len(job_postings_data)} ofertas encontradas, {saved_jobs} guardadas, {matches_found} matches encontrados', 'success')
        ...
        logger.info(f"Scraping con PLAYWRIGHT completado para usuario {user_id}: {result}")
        return result
