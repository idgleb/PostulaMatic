...
def check_and_send_auto_emails(self):
    ...
    logger.info(f"👥 Encontrados {users_with_auto_send.count()} usuarios con envío automático")
    ...
    for profile in users_with_auto_send:
        ...
        logger.info(f"📬 Usuario {user.username}: {len(new_matches)} nuevos matches encontrados")
...
def find_new_matches(user: User, profile: UserProfile):
    ...
    matches_above_threshold = MatchScore.objects.filter(
        user=user,
        score__gte=profile.match_threshold
...
def send_auto_emails_for_user(self, user_id: int):
    ...
    try:
        ...
        if not profile.auto_send_enabled:
            ...
            return {
                'success': False,
                'error': 'Envío automático desactivado'
            }
