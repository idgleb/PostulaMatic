...
from .models import UserProfile, JobPosting, MatchScore, UserCV
from .clients.dvcarreras_advanced import DVCarrerasAdvancedClient, JobPostingData
...
def scrape_dvcarreras_jobs_advanced(self, user_id: int):
    ...
    try:
        ...
        if not user_profile.dv_username or not user_profile.dv_password:
            logger.error(f"Usuario {user_id} no tiene credenciales de dvcarreras configuradas")
        ...
        with DVCarrerasAdvancedClient(
            ...
            for job_data in job_postings_data:
                try:
                    ...
                    if created:
                        new_jobs += 1
                        logger.info(f"Nueva oferta guardada: {job_posting.title} en {job_posting.company}")
