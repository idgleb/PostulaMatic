...
def test_ai_provider_view(request):
    ...
    try:
        ...
        if provider not in ai_email_service.get_available_providers():
            return JsonResponse(
                {"success": False, "message": f"Proveedor no disponible: {provider}"}
            )
        ...
        response_data = {
            ...
            "email_data": {"subject": result.subject, "body": result.body},
        }
...
def update_ai_settings_view(request):
    ...
    try:
        ...
        if provider not in ai_email_service.get_available_providers():
            return JsonResponse(
                {"success": False, "message": f"Proveedor no válido: {provider}"}
            )
