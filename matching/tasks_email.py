...
def send_personalized_email_task(
    ...
    try:
        ...
        logger.info(
            f"Iniciando envío de email para usuario {user_id}, CV {cv_id}, puesto {job_id}"
        )
...
def send_bulk_emails_task(
    ...
    try:
        ...
        for i in range(0, len(job_ids), batch_size):
            ...
            logger.info(
                f"Procesando batch {i//batch_size + 1} con {len(batch)} puestos"
            )
            ...
            for job_id in batch:
                try:
                    ...
                    results.append(
                        {"job_id": job_id, "task_id": result.id, "status": "queued"}
                    )
            ...
            if i + batch_size < len(job_ids):
                logger.info(
                    f"Esperando {delay_between_batches} segundos antes del siguiente batch"
                )
...
def process_matching_and_send_emails_task(
    ...
    try:
        ...
        high_score_matches = MatchScore.objects.filter(
            cv__user=user, score__gte=min_match_score
        ...
        return {"success": False, "error": f"Error procesando matching: {str(e)}"}
...
def generate_personalized_email(
    ...
    try:
        ...
        return {"success": True, "email_content": result, "user_profile": user_profile}
...
def send_email_with_attachments(
    user: User, job_posting: JobPosting, email_result: Dict, cv_result: Dict
) -> Dict:
...
def get_user_smtp_connection(user_profile: UserProfile):
    ...
    logger.info(
        f"Configurando SMTP para {user_profile.smtp_username} en {user_profile.smtp_host}:{user_profile.smtp_port}"
    )
...
def check_daily_email_limit(user: User) -> Dict:
    ...
    try:
        ...
        sent_today = EmailSentLog.objects.filter(user=user, sent_at__date=today).count()
        ...
        if not can_send:
            ...
            retry_after = timezone.make_aware(
...
def record_email_sent(
    ...
    try:
        ...
        logger.info(
            f"Email registrado en log para usuario {user.id}, puesto {job_posting.id}"
        )
...
def cleanup_old_email_logs_task(days_to_keep: int = 30):
    ...
    try:
        ...
        deleted_count = EmailSentLog.objects.filter(sent_at__lt=cutoff_date).delete()[0]
...
def send_email_reminder_task(user_id: int, days_since_last_activity: int = 7):
    ...
    try:
        ...
        recent_activity = EmailSentLog.objects.filter(
            user=user, sent_at__gte=last_activity
        ).exists()
