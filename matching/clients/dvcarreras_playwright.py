...
class DVCarrerasPlaywrightClient:
    ...
    async def scrape_job_board(self, max_pages: int = 3) -> List[JobPostingData]:
        ...
        try:
            logger.info(f"Iniciando scraping del tablero de ofertas (máx {max_pages} páginas)")
    ...
    async def _extract_jobs_from_page(self) -> List[JobPostingData]:
        ...
        try:
            ...
            await self.page.wait_for_selector('.job-posting, .job-card, .offer, [class*="job"]', timeout=10000)
