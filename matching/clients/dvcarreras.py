...
class DVCarrerasClient:
    ...
    def __init__(self, username: str, password: str, rate_limit_delay: Tuple[float, float] = (1.0, 3.0)):
        """
    ...
    def login(self) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Iniciando login para usuario: {self.username} (intento {attempt + 1}/{max_retries})")
                ...
                logger.debug(f"Respuesta del login: {response.status_code} - {response.reason}")
                
                if response.status_code == 403:
                    logger.error(f"Acceso denegado (403) a {self.LOGIN_URL}. Intento {attempt + 1}/{max_retries}")
                    if attempt < max_retries - 1:
                        ...
                        logger.error("Todos los intentos de login fallaron con error 403")
                        logger.error("- Posibles causas: Rate limiting, User-Agent bloqueado, o protección anti-bot")
                        return False
                ...
                username_field = soup.find('input', {'name': 'username'}) or soup.find('input', {'name': 'user'})
                password_field = soup.find('input', {'name': 'password'}) or soup.find('input', {'name': 'pass'})
                
                if not username_field or not password_field:
                    logger.error("No se encontraron campos de usuario/contraseña en el formulario")
                    if attempt < max_retries - 1:
                        continue
                    return False
                ...
                self._random_delay()
                login_response = self.session.post(login_url, data=form_data, timeout=30, allow_redirects=True)
                ...
                if self._is_login_successful(login_response):
                    ...
                    logger.error(f"Login fallido - credenciales incorrectas o error en el formulario (intento {attempt + 1})")
                    if attempt < max_retries - 1:
                ...
                logger.error(f"Error de conexión durante login (intento {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                ...
                logger.error(f"Error inesperado durante login (intento {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                    continue
                return False
    ...
    def scrape_job_board(self, max_pages: int = 5) -> List[JobPostingData]:
        ...
        try:
            for page in range(max_pages):
                ...
                logger.info(f"Página {page + 1}: {len(page_jobs)} ofertas encontradas, {new_count} nuevas, {duplicate_count} duplicadas")
            ...
            logger.info(f"Scraping completado. Total: {len(job_postings)} nuevas, {duplicate_count} duplicadas omitidas")
            return job_postings
    ...
    def _parse_job_board_page(self, html_content: str, page_url: str) -> List[JobPostingData]:
        """
