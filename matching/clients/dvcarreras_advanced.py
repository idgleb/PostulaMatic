...
class DVCarrerasAdvancedClient:
    ...
    def _rotate_user_agent(self):
        ...
        logger.info(f"Rotando User-Agent: {old_ua[:50]}... -> {self._current_user_agent[:50]}...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt > 0:
                    ...
                    logger.info(f"Esperando {delay:.1f} segundos antes del reintento {attempt + 1}")
                    time.sleep(delay)
                ...
                if response.status_code == 403:
                    logger.warning(f"Error 403 en intento {attempt + 1}. Rotando configuración...")
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            if main_response.status_code != 200:
                logger.error(f"Error accediendo a página principal: {main_response.status_code}")
                return False
            ...
            if login_page_response.status_code != 200:
                logger.error(f"Error accediendo a página de login: {login_page_response.status_code}")
                return False
            ...
            login_response = self._make_request('POST', login_url, data=form_data, allow_redirects=True)
            ...
            if self._is_login_successful(login_response):
                ...
                logger.error("Login fallido - credenciales incorrectas o error en el formulario")
                return False
    ...
    def _is_login_successful(self, response: requests.Response) -> bool:
        ...
        success_indicators = [
            'dashboard', 'panel', 'welcome', 'bienvenido',
            'logout', 'salir', 'profile', 'perfil'
        ]
    ...
    def scrape_job_board(self, max_pages: int = 3) -> List[JobPostingData]:
        ...
        try:
            logger.info(f"Iniciando scraping avanzado del tablero (máx {max_pages} páginas)")
            ...
            for page_num in range(max_pages):
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Encontradas {len(page_jobs)} ofertas en página {page_num + 1}")
                else:
                    logger.error(f"Error accediendo a página {page_num + 1}: {response.status_code}")
                
                # Pausa entre páginas
                if page_num < max_pages - 1:
                    self._random_delay(5, 10)
            
            logger.info(f"Scraping avanzado completado: {len(job_postings)} ofertas encontradas")
            return job_postings
    ...
    def _extract_jobs_from_page(self, html_content: str) -> List[JobPostingData]:
        ...
        try:
            ...
            job_elements = soup.find_all(['div', 'article', 'section'], class_=re.compile(r'job|offer|posting', re.I))
            ...
            for index, element in enumerate(job_elements):
                try:
                    ...
                    company_elem = element.find(['span', 'div'], class_=re.compile(r'company', re.I))
                    company = company_elem.get_text(strip=True) if company_elem else ''
                    
                    location_elem = element.find(['span', 'div'], class_=re.compile(r'location', re.I))
                    location = location_elem.get_text(strip=True) if location_elem else ''
                    
                    description_elem = element.find(['div', 'p'], class_=re.compile(r'description', re.I))
