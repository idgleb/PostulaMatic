...
from ..services.flaresolverr_client import get_flaresolverr_client, test_flaresolverr_connection
...
class DVCarrerasPlaywrightFlareSolverr:
    ...
    def __init__(self, username: str, password: str, headless: bool = True, log_callback=None, session_dir: str = None):
        """
    ...
    async def load_session(self) -> bool:
        ...
        try:
            ...
            if session_age.days > 7:
                await self._log("Sesión guardada es muy antigua, se requiere nuevo login", 'warning')
                return False
            ...
            if cookies and self.context:
                ...
                await self._log(f"Cargadas {len(cookies)} cookies de sesión guardada", 'success')
    ...
    async def test_session_validity(self) -> bool:
        ...
        try:
            ...
            await self.page.goto(self.JOB_BOARD_URL, wait_until='domcontentloaded', timeout=15000)
            ...
            if "Just a moment" in page_content or "cloudflare" in page_content.lower() or "Un momento" in page_content:
                await self._log("Cloudflare detectado en validación - usando FlareSolverr", 'info')
            ...
            success_indicators = ['logout', 'salir', 'perfil', 'profile', 'ofertas', 'jobs', 'tablero']
            ...
            if 'dvcarreras.davinci.edu.ar' in current_url and 'login' not in current_url.lower():
                await self._log("Sesión probablemente válida - no en página de login", 'success')
                self._is_authenticated = True
                return True
    ...
    async def test_login(self) -> bool:
        ...
        """
        try:
            await self._log(f"Probando login con FlareSolverr para usuario: {self.username}", 'info')
            ...
            if await self.load_session():
                ...
                if await self.test_session_validity():
                    await self._log("Sesión válida encontrada, login exitoso", 'success')
                    
                    # Hacer scraping inmediatamente mientras tenemos la sesión activa
                    await self._log("Realizando scraping inmediato con sesión activa...", 'info')
                    jobs = await self.scrape_job_board_immediate()
                    await self._log(f"Scraping inmediato completado: {len(jobs)} ofertas encontradas", 'success')
                    
                    return True
                else:
                    await self._log("Sesión cargada no es válida, procediendo con login completo", 'warning')
            else:
                await self._log("No hay sesión válida, procediendo con login completo", 'info')
            
            # Primero verificar que FlareSolverr esté funcionando
            if not self.flaresolverr.is_healthy():
                await self._log("FlareSolverr no está funcionando correctamente", 'error')
                return False
            
            await self._log("FlareSolverr está funcionando, procediendo con login", 'info')
            ...
            if cookies:
                await self.context.add_cookies(cookies)
                await self._log(f"Configuradas {len(cookies)} cookies en Playwright", 'info')
            ...
            page_content = await self.page.content()
            if "Just a moment" in page_content or "cloudflare" in page_content.lower() or "Un momento" in page_content:
                await self._log("Cloudflare detectado, usando FlareSolverr para resolver...", 'warning')
                
                # Usar FlareSolverr para resolver la página completa
                solved_result = self.flaresolverr.solve_page(self.LOGIN_URL)
                
                if solved_result and solved_result.get("status") == "ok":
                    await self._log("Cloudflare resuelto exitosamente con FlareSolverr", 'success')
                    ...
                    await self._log("Error resolviendo Cloudflare con FlareSolverr", 'error')
                    return False
            
            # Paso 4: Buscar campos de login
            await self._log("Buscando campos de login...", 'info')
            username_field = await self.page.query_selector('input[name="user"], input[name="username"], input[type="text"]')
            password_field = await self.page.query_selector('input[type="password"]')
            
            if not username_field or not password_field:
                await self._log("No se encontraron campos de usuario/contraseña", 'error')
                return False
            ...
            login_button = await self.page.query_selector('button:has-text("Acceder"), button[type="submit"], input[type="submit"]')
            ...
            try:
                ...
                page_content = await self.page.content()
                if "Just a moment" in page_content or "cloudflare" in page_content.lower() or "Un momento" in page_content:
                    await self._log("Cloudflare detectado después del login, usando FlareSolverr para resolver...", 'warning')
                    ...
                    if solved_result and solved_result.get("status") == "ok":
                        await self._log("Cloudflare post-login resuelto exitosamente", 'success')
                        ...
                        await self._log("Error resolviendo Cloudflare post-login", 'error')
                
                await asyncio.sleep(2)
                
            except Exception as nav_error:
                await self._log(f"Timeout en navegación, continuando: {nav_error}", 'warning')
                await asyncio.sleep(2)
    ...
    async def scrape_job_board_immediate(self, max_pages: int = 1) -> List[Dict[str, Any]]:
        """
        ...
        if not self._is_authenticated or not self.page:
            logger.error("No se puede scrapear sin estar autenticado o sin página activa")
            return []
        ...
        try:
            ...
            await self.page.goto(self.JOB_BOARD_URL, wait_until='domcontentloaded', timeout=30000)
            ...
            if 'login' in current_url.lower():
                await self._log("❌ Redirigido a página de login - sesión perdida", 'error')
                return []
            ...
            page_content = await self.page.content()
            await self._log(f"Contenido de la página: {len(page_content)} caracteres", 'info')
            
            # Verificar si hay un challenge de Cloudflare
            if 'turnstile' in page_content.lower() or 'verificar que usted es un ser humano' in page_content.lower():
                await self._log("❌ Cloudflare Turnstile detectado - esperando resolución...", 'warning')
                
                # Esperar más tiempo para que se resuelva automáticamente
                await self._log("Esperando 10 segundos para resolución automática...", 'info')
                ...
                await self._log(f"Después de esperar - URL: {current_url}, Título: {page_title}", 'info')
                
                # Verificar si aún hay Turnstile
                if 'turnstile' in page_content.lower() or 'verificar que usted es un ser humano' in page_content.lower():
                    await self._log("❌ Turnstile aún presente - no se pudo resolver automáticamente", 'error')
                    ...
                    await self._log(f"Contenido del challenge guardado en: {debug_filename}", 'info')
                    return []
                else:
                    await self._log("✅ Turnstile resuelto automáticamente", 'success')
            
            # Verificar si hay contenido de ofertas
            if 'oferta' not in page_content.lower() and 'trabajo' not in page_content.lower():
                await self._log("❌ No se encontró contenido de ofertas en la página", 'warning')
                # Guardar el contenido para debugging
            ...
            await self._log(f"Encontradas {len(jobs_data)} ofertas en el tablero", 'success')
            return jobs_data
    ...
    async def scrape_job_board(self, max_pages: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            await self._log(f"Cookies disponibles para FlareSolverr: {len(current_cookies)}", 'info')
            
            # Debug: mostrar las cookies que se van a enviar
            for cookie in current_cookies:
                await self._log(f"Cookie: {cookie.get('name')} = {cookie.get('value')[:20]}...", 'info')
            
            # Usar FlareSolverr para resolver la página del tablero con cookies
            await self._log(f"Enviando petición a FlareSolverr para: {self.JOB_BOARD_URL}", 'info')
            solved_result = self.flaresolverr.solve_page(self.JOB_BOARD_URL, cookies=current_cookies)
            
            await self._log(f"Respuesta de FlareSolverr: {solved_result}", 'info')
            
            if not solved_result or solved_result.get("status") != "ok":
                await self._log("Error resolviendo página del tablero con FlareSolverr", 'error')
            ...
            await self._log(f"HTML recibido de FlareSolverr: {len(solved_html)} caracteres", 'info')
    ...
    def test_login_sync(self) -> bool:
        ...
        """
        try:
            logger.info(f"Probando login síncrono con FlareSolverr para usuario: {self.username}")
