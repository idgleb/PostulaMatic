...
def send_bulk_emails_task(
    ...
    logger.info(f"📋 Total puestos: {len(job_ids)}, Lote: {batch_size}, Delay: {delay_between_batches}s")
    ...
    try:
        ...
        if cv_id:
            try:
                ...
                cv_name = user_cv.original_file.name.split('/')[-1] if user_cv.original_file else f'CV #{cv_id}'
                ...
                return {
                    'success': False,
                    'error': f'CV con ID {cv_id} no encontrado'
                }
        else:
            user_cv = UserCV.objects.filter(user=user).order_by('-created_at').first()
            if user_cv:
                cv_name = user_cv.original_file.name.split('/')[-1] if user_cv.original_file else f'CV #{user_cv.id}'
        ...
        batches = [job_ids[i:i + batch_size] for i in range(0, total_jobs, batch_size)]
        ...
        for batch_num, batch_job_ids in enumerate(batches, 1):
            logger.info(f"📦 Procesando lote {batch_num}/{len(batches)} ({len(batch_job_ids)} puestos)")
            ...
            for job_id in batch_job_ids:
                try:
                    ...
                    if not job_posting.email:
                        logger.warning(f"⚠️ Puesto {job_posting.title} sin email, saltando")
                        results['failed'] += 1
                        _create_email_log(
                            user=user,
                            cv=user_cv,
                    ...
                    if not personalization_result.get('success'):
                        logger.error(f"❌ Error personalizando CV: {personalization_result.get('error')}")
                        results['failed'] += 1
                        _create_email_log(
                            user=user,
                            cv=user_cv,
                    ...
                    logger.info(f"🤖 Proveedor IA usado realmente: {actual_ai_provider}")
                    ...
                    if send_result['success']:
                        logger.info(f"✅ Email enviado exitosamente a {job_posting.email}")
                        results['sent'] += 1
                        _create_email_log(
                            user=user,
                            cv=user_cv,
            ...
            if batch_num < len(batches):
                logger.info(f"⏱️ Esperando {delay_between_batches}s antes del siguiente lote...")
...
def _create_email_log(user, cv, job_posting, status, sent_to='', email_subject='', email_body='', error_message='', task_id='', email_template='base', ai_provider='openai', pdf_buffer=None):
    ...
    try:
        ...
        if pdf_buffer and status == 'sent':
            try:
                ...
                job_title_safe = "".join(c for c in (job_posting.title if job_posting else "puesto")[:30] if c.isalnum() or c in (' ', '-', '_')).strip()
