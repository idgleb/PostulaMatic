...
class ProgressTracker:
    ...
    STEPS = [
        ...
        {"id": "temp_file", "label": "💾 Guardando archivo temporal", "status": "pending"},
        {"id": "pdf_to_images", "label": "🖼️ Convirtiendo PDF a imágenes", "status": "pending"},
        {"id": "openai_vision", "label": "🤖 Extrayendo texto con OpenAI", "status": "pending"},
        {"id": "anthropic_vision", "label": "🔄 Fallback a Anthropic", "status": "pending"},
        {"id": "skills_extraction", "label": "🎯 Extrayendo habilidades", "status": "pending"},
        {"id": "db_save", "label": "💾 Guardando en base de datos", "status": "pending"},
    ...
    def update_step(self, step_id, status, message=None, details=None):
        ...
        logger.info(f"📊 Progress actualizado [{self.progress_id}]: {step_id} -> {status}")
    ...
    def set_error(self, error_message, step_id=None):
        ...
        cache.set(self.cache_key, progress_data, self.cache_timeout)
        logger.error(f"❌ Progress marcado como error [{self.progress_id}]: {error_message}")
