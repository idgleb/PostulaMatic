...
async def debug_job_board():
    ...
    try:
        ...
        async with DVCarrerasPlaywrightSimple(username=username, password=password) as client:
                ...
                try:
                    ...
                    if len(elements) > 0:
                        ...
                        for i, element in enumerate(elements[:3]):  # Solo los primeros 3
                            ...
                            if text and len(text.strip()) > 10:  # Solo elementos con contenido significativo
                                logger.info(f"  Elemento {i+1}: {text.strip()[:100]}...")
            ...
            if 'oferta' in page_text.lower() or 'trabajo' in page_text.lower() or 'empleo' in page_text.lower():
                logger.info("La página contiene palabras relacionadas con ofertas de trabajo")
            else:
                logger.warning("La página NO contiene palabras relacionadas con ofertas de trabajo")
